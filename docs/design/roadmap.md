...
| P0.4 | Intent/state correspondence + `effective_config` model | this repo | **partial** — `observe` returns the structural `effective_config` projection (config-surface subset of state, comparable to intent); semantic correspondences (inversions, name↔ID, defaults) and host-side `satisfies` (P4.1) remain | [state-model](specs/state-model.md) |
...
| P2.3 | Model → CIC provider contract + module type generator | this repo | **in progress** — field-policy + payload-schema emission done (`tools/oci-extract/{policy,schema}.go`); generated schemas for **two** kinds embedded into the guest (`module/schemas/{vcn,subnet}.json`, `make oci.generate`) and consumed by `validate`/`plan`, proving the pipeline is not resource-specific; native module-type structs todo | [oci-schema-pipeline](specs/oci-schema-pipeline.md) |
| P2.4 | Schema diff / breaking-change gate on SDK bump | this repo | **done** — integrity gate (`extracted_schema_hash` = sha256 of the committed schema, checked in `test_oci_sdk_lock.py`) + semantic `oci-extract -diff` classifying breaking vs compatible (exit 3 on breaking) | [oci-schema-pipeline](specs/oci-schema-pipeline.md) |
